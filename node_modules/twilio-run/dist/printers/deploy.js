...
function plainPrintDeployedResources(config, result) {
    const functionsOutput = (0, columnify_1.default)(result.functionResources.sort(sortByAccess).map((fn) => (Object.assign(Object.assign({}, fn), { url: `https://${result.domain}${fn.path}` }))), {
        columns: ['access', 'path', 'url'],
        showHeaders: false,
    });
    const assetsOutput = (0, columnify_1.default)(result.assetResources.sort(sortByAccess).map((asset) => (Object.assign(Object.assign({}, asset), { url: `https://${result.domain}${asset.path}` }))), {
        columns: ['access', 'path', 'url'],
        showHeaders: false,
    });
    const data = {
        ...
        viewLiveLogs: (0, utils_1.getTwilioConsoleDeploymentUrl)(result.serviceSid, result.environmentSid),
    };
    const output = `
...
function prettyPrintDeployedResources(config, result) {
    const twilioConsoleLogsLink = (0, terminal_link_1.default)('Open the Twilio Console', (0, utils_1.getTwilioConsoleDeploymentUrl)(result.serviceSid, result.environmentSid), {
  ...
  `.trim());
    if (result.functionResources) {
        const functionMessage = result.functionResources
            .sort(sortByAccess)
            .map((fn) => {
            const accessPrefix = fn.access !== 'public' ? (0, chalk_1.default) `{bold [${fn.access}]} ` : '';
    ...
    if (result.assetResources) {
        const assetMessage = result.assetResources
            .sort(sortByAccess)
            .map((asset) => {
            const accessPrefix = asset.access !== 'public' ? (0, chalk_1.default) `{bold [${asset.access}]} ` : '';
