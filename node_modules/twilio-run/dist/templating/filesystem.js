...
function writeEnvFile(contentUrl, targetDir, functionName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const envFileExists = yield (0, fs_1.fileExists)(envFilePath, true);
        if (!envFileExists) {
            yield (0, fs_1.downloadFile)(contentUrl, envFilePath);
            return { newEnvironmentVariableKeys: [] };
        }
        const currentContent = yield (0, fs_1.readFile)(envFilePath, 'utf8');
        const currentFlags = dotenv_1.default.parse(currentContent);
        const newContent = (yield (0, got_1.default)(contentUrl)).body;
...
`;
        ...
        yield (0, fs_1.writeFile)(envFilePath, contentToWrite, 'utf8');
        return { newEnvironmentVariableKeys: functionKeys };
    });
}
function installDependencies(contentUrl, targetDir) {
    return __awaiter(this, void 0, void 0, function* () {
        const pkgContent = yield (0, got_1.default)(contentUrl, { responseType: 'json' });
        const dependencies = {};
        ...
        if (exactDependencies && Object.keys(exactDependencies).length > 0) {
            yield (0, pkg_install_1.install)(exactDependencies, {
                cwd: targetDir,
                exact: true,
            });
        }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (functionsTargetDir !== functionsDir) {
            if (hasFilesOfType(files, 'functions')) {
                yield (0, fs_1.mkdir)(functionsTargetDir, { recursive: true });
            }
            if (hasFilesOfType(files, 'assets')) {
                yield (0, fs_1.mkdir)(assetsTargetDir, { recursive: true });
            }
        }
        if (hasFilesOfType(files, 'README.md')) {
            yield (0, fs_1.mkdir)(readmesTargetDir, { recursive: true });
        }
        for (let file of files) {
            if (file.type === 'functions') {
                let filepath = path_1.default.join(functionsTargetDir, file.directory, file.name);
                if (yield (0, fs_1.fileExists)(filepath)) {
        ...
        const tasks = files
            ...
            if (file.type === 'functions') {
                ...
                return {
                    ...
                    task: () => __awaiter(this, void 0, void 0, function* () {
                        if (yield (0, fs_1.fileExists)(readmePath)) {
                            return;
                        }
            ...
            .filter(Boolean);
        const context = yield new listr2_1.Listr(tasks, { concurrent: true }).run();
